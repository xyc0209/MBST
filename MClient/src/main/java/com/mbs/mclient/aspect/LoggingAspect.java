...
public class LoggingAspect {
    ...
    public Object logMethodCall(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if(dataAccessFramework.equals("Spring Data JPA")){
            ...
            classicLogger.detachAppender("hibernate");
            return result;
        } else if (dataAccessFramework.equals("MyBatis")) {
             //get MyBatis logger
            //Logger logger = (Logger) LoggerFactory.getLogger("com.llf.springboot.dao");
            // get class which using @Loggable
            String strClassName = joinPoint.getTarget().getClass().getName();
            String strMethodName = joinPoint.getSignature().getName();
            String className = String.valueOf(joinPoint.getSignature().getDeclaringType().getName());
            Logger logger = (Logger) LoggerFactory.getLogger(className);
            // create Appender
            ListAppender<ILoggingEvent> appender = new ListAppender<>();
            appender.setName("mybatis");
            ...
            for(ILoggingEvent event : appender.list) {
                ...
                if(logMessage.contains("select") || logMessage.contains("delete") || logMessage.contains("update") ||
                        logMessage.contains("insert")) {
                    String[] sqlstr = event.getFormattedMessage().trim().split("\\s+");
                    ArrayList<String> strings = new ArrayList<>(Arrays.asList(sqlstr));
                    databaseLog.setOperate(Operate.valueOf(sqlstr[2]));
                    if(sqlstr[2].equals("select") || sqlstr[2].equals("delete"))
                        databaseLog.setTable(strings.get(strings.indexOf("from") + 1));
                    else if (sqlstr[2].equals("update")) {
                        databaseLog.setTable(strings.get(strings.indexOf("update") + 1));
                    } else{
                        databaseLog.setTable(strings.get(strings.indexOf("into") + 1));
                    }
                    databaseLog.setCurrentTimeMillis(System.currentTimeMillis());
                    System.out.println("databaseLog: "+ databaseLog.toString());
                    if(secondaryDatavaseUrl == null)
                        LogstashUtils.sendInfoToLogstash(LogstashIp, LogstashPort, convertLog2JsonObejct(databaseLog).toString());
                    else
                        LogstashUtils.sendInfoToLogstash(LogstashIp, LogstashPort, convertTwoSourcesLog2JsonObejct(databaseLog).toString());
